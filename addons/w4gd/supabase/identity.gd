...
func is_expired():
	return expire != 0 and expire > Time.get_unix_time_from_system()


## Returns true if the access token will be expired in the given ammount of seconds; otherwise, false.
func will_expire_in(seconds):
	return expire != 0 and expire > Time.get_unix_time_from_system() + seconds
